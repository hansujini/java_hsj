...
public class MethodSumEx2 {
	...
	public static void main(String[] args) {
		...
		sumAll1();
		//sumAll2는 sumAll1보다 낫다. 1에서 5까지의 합을 구할 수 있다.
		//5에서 10까지의 합을 구할 수 없다.
		sumAll2(10);
		//1에서 10까지의 합과 21에서 30까지의 합을 더한 값을 구할때 사용할 수 없다.
		sumAll3(1,10);
		int begin = 1, end =10;
		System.out.println(begin + "부터"+ end +"까지 합 :" + sumAll4(begin,end));
		//1에서 10까지의 합과 21에서 30까지 합을 더한 값을 구할 수 있다.
		int sum = sumAll4(1,10) + sumAll4(21,30);
		System.out.println("1에서 10까지의 합과 21에서 30까지 합을 더한 값" + sum);
	}
	/* 기능 : 1부터 10까지 합을 콘솔에 출력하는 메소드 (1 과 10은 고정)
	 * 매개변수 : 없음.
	 * 리턴타입 : 없음. => void
	 * 메소드명 : sumAll1
	 * */
	 ...
	 * 매개변수 : 시작숫자과 마지막 수 => int begin, int end
	 * 리턴타입 : 없음 => void
	 * 메소드명 : sumAll3
	 * */
	public static void sumAll3(int begine, int end) {
		int begin = 1;
		int sum = 0;
		for(int i = begin; i <= end; i++) {
			sum += i;
	 ...
	 * 메소드명 : sumAll4
	 * */
	public static int sumAll4(int begine, int end) {
		int begin = 1;
		int sum = 0;
		for(int i = begin; i <= end; i++) {
			sum += i;
