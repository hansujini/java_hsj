...
public class LottoEx1 {
	...
	public static void main(String[] args) {
		...
		//당첨번호와 중복되지 않게 보너스 번호를 램덤으로 생성하여 저장
	...
	public static int [] randomArray(int min, int max, int size) {
		...
		for(int count = 0; count<size; ) {
			...
			//중복체크 random과 배열을 비교하여 중복된 숫자가 없으면
	...
	/* 기능 : 최소값(min), 최대값(max), 배열이 주어지면 min~max사이의 랜덤한 수를 생성하여 배열에
	 * 있는지 없는지 확인 후 없으면 해당 수를 알려주는 메소드
	...
	public static int createBonus(int min, int max, int []arr) {
		...
		while(true) {
			...
			//당첨번호에 보너스 번호가 있으면 조건식으로 점프(이동)하여 다시 
			//번호 생성
	...
	/*기능 : 두 배열이 주어지면, 두 배열에서 일치하는 정수의 갯수를 알려주는 매소드
	 * 매개변수 : 두배열 = int []arr2, int []arr2
